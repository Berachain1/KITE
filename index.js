...
async function main() {
  ...
  let accounts = [];
  try {
    const accountsData = await fs.readFile('annisa.txt', 'utf8');
    ...
    console.log(chalk.red('✗ File annisa.txt tidak ditemukan atau kosong! Pastikan berisi privateKey per baris.'));
    rl.close();
    return;
  }

  if (accounts.length === 0) {
    console.log(chalk.red('✗ Tidak ada akun valid di annisa.txt!'));
    rl.close();
    return;
  }
